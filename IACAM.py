...
class YOLOApp(QMainWindow):
    ...
    # Verificar se há fonte de vídeo/câmera
      ...
      # Variáveis para contagem de veículos
      car_count = 0
      truck_count = 0
      ...
      if self.yolo_active and len(self.video_label.roi_points) > 2 and self.frame_count % self.process_every_n_frames == 0:
          ...
          for box in results.boxes:
              ...
              if np.sum(intersecao) > 0:
                  ...
                  # Identificar e contar veículos
                  if classe == "carro":
                      car_count += 1
                      current_vehicle_type = "Carro"
                      color = (0, 255, 0)  # Verde para carros
                  elif classe == "caminhao":
                      truck_count += 1
                      current_vehicle_type = "Caminhão"
                      color = (0, 0, 255)  # Vermelho para caminhões
                  else:
                      color = (255, 0, 0)  # Azul para outras classes

                  # Lógica de contagem de eixos e rodas
                  if classe == "pneu":
                      total_eixos_normais += 1
                      total_rodas += 2
                  elif classe == "eixo_suspenso":
                      total_eixos_erguidos += 1
                      total_eixos_normais += 1
                      total_rodas += 2

                  # Desenhar bbox e texto
                  cv2.rectangle(frame_to_show, (x1, y1), (x2, y2), color, 2)
                  cv2.putText(frame_to_show, f"{classe} {conf:.2f}", (x1, y1-10),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)

          # Adicionar informações de veículos no frame
          # Contagem de veículos
          veiculo_texto = f"Cars: {car_count} | Trucks: {truck_count}"
          cv2.putText(frame_to_show, veiculo_texto, (10, 30), 
                      cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)

          # Informações de eixos e rodas
          eixos_texto = f"Eixos: {total_eixos_normais} (Normais: {total_eixos_normais - total_eixos_erguidos}, Suspensos: {total_eixos_erguidos})"
          rodas_texto = f"Rodas: {total_rodas}"
          
          cv2.putText(frame_to_show, eixos_texto, (10, 60), 
                      cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
          cv2.putText(frame_to_show, rodas_texto, (10, 90), 
